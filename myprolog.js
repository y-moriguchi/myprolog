...
function makeProlog() {
    ...
    function makeOperatorMatcher(follow) {
        return function(str, index) {
            var result = operator.parse(str, index, follow);
    ...
    parser = R.or(
        R.action(/[a-z][a-zA-Z0-9_]*/, function(name) { return makeSymbol(name); }),
        R.action("!", function(name) { return makeSymbol("!"); }),
        R.action(/[0-9]+/, function(num) { return makeNumber(parseInt(num)); }),
        R.action(/[A-Z_][a-zA-Z0-9_]*/, function(name) { return name === "_" ? makeAnonymousVariable() : makeVariable(name); }),
        R.action(/[\+\-\*\/<>=:\.&_~\^\\@]+/, function(name) { return makeSymbol(name); }),
        R.action(/\'[^\']+\'/, function(name) { return makeSymbol(name.substring(1, name.length - 1)); }),
        R.then(
            "[",
            R.or(
                R.action("]", function() { return makeSymbol("[]"); }),
                R.then(
                    R.attr([]),
                    R.delimit(
                        makeOperatorMatcher(/[,\|\]]/),
                    ...
                    R.or(
                        R.action(
                            R.then(
                                "|",
                                R.action(makeOperatorMatcher(/[\]]/), function(x, result, list) { return list.concat([result]); }),
                                "]"),
                            function(x, syn, list) { return arrayToPrologList(list.slice(0, list.length - 1), list[list.length - 1]); }),
                        R.action("]", function(x, syn, list) { return arrayToPrologList(list); }))))));

    parserTerm = R.action(
        R.then(
            R.action(/[a-z][a-zA-Z0-9_]*|[\+\-\*\/<>=:\.&_~\^\\@]+/, function(name) { return { name: name, args: [] }; }),
            "(",
            R.delimit(
                makeOperatorMatcher(/[,\)]/),
                ",",
            ...
            ")"),
        function(match, x, inherit) { return makeCompoundTerm(x.name, x.args); });

    operator = K.Operator({
        id: function(str, index) {
            var result = parser(str, index);
        ...
        preId: function(str, index) {
            var result = parserTerm(str, index);
